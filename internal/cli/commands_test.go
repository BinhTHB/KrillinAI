...
func TestFixGeminiDubSRTTimingUsesShortOriginForMonotonicOverrun(t *testing.T) {
	dir := t.TempDir()
	badOrigin := `1
00:00:00,000 --> 00:00:01,000
一
...
`
	...
	if fixed[0].Start != 0 || fixed[2].Start != 5 || fixed[3].End > 10.001 {
		t.Fatalf("unexpected short-origin distributed timing: %#v", fixed)
	}
}
