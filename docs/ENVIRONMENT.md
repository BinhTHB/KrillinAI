...
|------|---------|-------------|
| `R2_ACCESS_KEY_ID` | `R2Client` | Cloudflare R2 access key ID. Sensitive. |
| `R2_SECRET_ACCESS_KEY` | `R2Client` | Cloudflare R2 secret access key. Sensitive. |
...
| `YT_DLP_COOKIES` | `IngestWorkflow` | Optional Netscape-format cookies text for `yt-dlp` authentication, needed for Douyin/TikTok links that require fresh cookies. Sensitive. |
...
| `R2_ENDPOINT` | `R2Client` | Cloudflare R2 S3-compatible endpoint. Non-sensitive. |
| `R2_BUCKET` | `R2Client` | Cloudflare R2 bucket name. Non-sensitive. |
...
| R2 bucket | Dev bucket via `R2_BUCKET` | Prod bucket via `R2_BUCKET` |
