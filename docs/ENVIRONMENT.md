...
| `WHISPER_MODEL` | `HuggingFaceClient` / HF Space | ASR model name, e.g. `distil-large-v3`. Non-sensitive. |
...
| Name | Description |
|------|-------------|
| `WHISPER_MODEL` | Faster-Whisper model to load, e.g. `distil-large-v3`. |
| `WHISPER_DEVICE` | Runtime device, e.g. `auto`, `cuda`, or `cpu`. |
| `WHISPER_COMPUTE_TYPE` | Compute type, e.g. `float16`, `int8_float16`, or `int8`. |
