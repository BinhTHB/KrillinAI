...
Method:
  ...
  - Speed up lightly, then clip TTS audio to fit original timeline (overlay mode) or allow spill (voiceover mode).
  - Build ASS/SRT aligned to the final video.
"""
...
async def main():
    ...
    parser.add_argument('--output-dir', default='controlled_tts_segment')
    ...
    parser.add_argument('--timeline-mode', default='overlay', choices=['overlay', 'voiceover'], help='overlay keeps original video timeline and clips TTS to cues; voiceover keeps original video and allows safe spill past cue end')
    ...
    if args.timeline_mode in ['overlay', 'voiceover']:
        ...
        for idx, chunk in enumerate(chunks):
            ...
            if args.keep_cache and tts_speed.exists() and get_duration(tts_speed) > 0.05:
                ...
                clipped = max(0.0, speed_dur - fit_dur)
                ...
                overflow = f" clipped={clipped:.2f}s" if clipped > 0.02 else ""
